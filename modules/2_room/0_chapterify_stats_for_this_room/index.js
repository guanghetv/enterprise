                    ...
                    _.each(chapterIdsArray, function (chapterId) {
                        chapterTaskGroup.push(function (callback) {
                            dataManager.cache.getHash(hashKey, chapterId, function (err, statsForThisChapter) {
                                var middleHashKey = "middle@room@$chapter_id";
                                dataManager.cache.setHashField(middleHashKey.replace('$chapter_id', chapterId), studentId, JSON.parse(statsForThisChapter), function (err, reply) {
                                    callback(err, 'OK');
                                });
                            })
                        })
